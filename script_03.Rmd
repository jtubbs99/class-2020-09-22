...
- We should filter by courses that have had a drop in enrollment number.
- We should filter by the dates we're interested in by using sep_old and sep_new.


After we have a model, Temperance suggests we should:

- Acknowledge the fact that even if the course enrollment number didn't drop, people could have dropped the course (and different people added it).
- We can't know why people chose to drop these courses, only the numbers of how many dropped.
...
```{r}
enrollment_changes <- merge(sep_old, sep_new) %>% 
  filter() %>% 
  head()
